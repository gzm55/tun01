  ...
  namespace net {
    ...
    class WriteStream {
     ...
     public:
      WriteStream() {}
      ~WriteStream() {
        for (std::deque<WritePacket*>::iterator i = m_packet_queue.begin(); i != m_packet_queue.end(); i++) {
          delete *i;
        }
      }
      bool pendingWrite() { return m_packet_queue.size() > 0; }

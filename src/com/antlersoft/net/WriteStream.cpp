...
WritePacket::WritePacket(deque<char>& queue) : m_size(queue.size() + 4), m_written(0) {
...
bool WritePacket::sendPacket(int socket) {
  ...
  ssize_t write_length = write(socket, m_buf + m_written, m_size - m_written);
  if (write_length < 0) throw MY_EXCEPTION_ERRNO;
...
void WriteStream::writeBytes(char* buffer, int length) { m_queue.insert(m_queue.end(), buffer, buffer + length); }
...
void WriteStream::send() {
  ...
  if (m_queue.size() > 0) m_packet_queue.push_back(new WritePacket(m_queue));
}
...
void WriteStream::empty(pollfd& poll_struct) {
  ...
  if ((poll_struct.revents & POLLOUT) && pendingWrite()) {
    ...
    if (!pendingWrite()) poll_struct.events &= ~POLLOUT;
  }
}

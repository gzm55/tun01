...
int poll(pollfd* pollfds, unsigned int nfds, int timeout) {
  ...
  for (int i = 0; i < nfds; ++i) {
    pollfd& fd = pollfds[i];
    if (max_fd < fd.fd) max_fd = fd.fd;
    fd.revents = 0;
    FD_SET(fd.fd, &exceptfds);
    if (fd.events & POLLIN) FD_SET(fd.fd, &readfds);
    if (fd.events & POLLOUT) FD_SET(fd.fd, &writefds);
  }
...
Poller::Poller()
    : m_pollfds(new pollfd[INITIAL_SIZE]),
      m_size(INITIAL_SIZE),
      m_timeout(-1),
      m_finishing(false),
      m_callback(static_cast<PollCallback*>(0)) {}

Poller::~Poller() { delete[] m_pollfds; }
...
int Poller::pollOnce() {
  ...
  if (poll_result < 0) {
    if (errno != EINTR) throw MY_EXCEPTION_ERRNO;
  }
  if (poll_result > 0) {
    for (int i = 0; i < m_orig_size; ++i) {
...
void Poller::start() {
  ...
  while (!m_finishing) {
    if (pollOnce() == 0) return;
    if (m_callback != static_cast<PollCallback*>(0)) m_callback->pollCallback(*this);
  }
...
void Poller::setTimeout(int timeout) { m_timeout = timeout; }

void Poller::requestFinish() { m_finishing = true; }

bool Poller::isFinished() { return m_finishing; }

void Poller::addPolled(Polled& to_add) {
  auto ptr = std::make_shared<PolledAndPollfd>();
  ptr->m_polled = &to_add;
...
vector<Poller::PolledAndPollfdPtr>::iterator Poller::findPolled(Polled& to_find, int fd) {
  ...
  for (i = m_polled.begin(); i != m_polled.end(); ++i) {
...
void Poller::removePolled(Polled& to_remove, int fd) {
  ...
  if (i != m_polled.end()) {
    ...
    if (index < m_orig_size) m_orig_size--;
    if (index <= m_index) m_index--;
    m_polled.erase(i);
  }
}

...
int SockBuffer::writeData( int write_fd)
{
	...
	write_count=write( write_fd, m_output_buf+m_output_position,
		BUFFER_SIZE-m_output_position);
	if ( write_count<0)
		throw MY_EXCEPTION_ERRNO;
...
void SockBuffer::pushOnBuffer( const char* bytes, int length)
{
	m_queue.insert( m_queue.end(), bytes, bytes+length);
}

...
#ifdef COM_OUT
  cerr << "Queue size " << m_queue.size() << " Queue contents: " << cerr_endl() << flush;
...
void ReadStream::fill(pollfd& poll_struct) {
  ...
  if (poll_struct.revents & POLLIN) {
    ssize_t read_len;
    if (m_read_count < 4) {
      read_len = read(poll_struct.fd, m_read_buf + m_read_count, 4 - m_read_count);
      if (read_len < 0) throw MY_EXCEPTION_ERRNO;
      if (read_len == 0) throw MY_EXCEPTION("End of file in ReadStream::fill");
#ifdef COM_OUT
      cerr << "Read size " << read_len << " read contents: " << cerr_endl() << flush;
    ...
    read_len = read(poll_struct.fd, m_read_buf + m_read_count, length + 4 - m_read_count);
#ifdef COM_OUT
    cerr << "Read size " << read_len << " read contents: " << cerr_endl() << flush;
    for (int i = 0; i != read_len; ++i) {
      int ch = m_read_buf[m_read_count + i];
...
#endif
    if (read_len < 0) {
      if (errno != EAGAIN) throw MY_EXCEPTION_ERRNO;
      read_len = 0;
    }
    m_read_count += read_len;
    if (m_read_count == length + 4) {
      m_queue.insert(m_queue.end(), m_read_buf + 4, m_read_buf + m_read_count);
      m_read_count = 0;
    }
  }
}

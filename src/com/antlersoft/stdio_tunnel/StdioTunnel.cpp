...
void ConnectionLink::polled( Poller& poller, pollfd& poll_struct)
{
	...
	if ( poll_struct.revents & POLLOUT)
	{
		...
		if ( ! m_buffer.unsentData())
			poller.setEvents( *this, m_socket, m_acknowledged ? POLLIN : 0);
	}
...
void ConnectionLink::setAcknowledged( bool ack, Poller& poller)
{
	...
	if ( ! m_closing)
	{
		int events=( m_acknowledged ? POLLIN : 0 ) | ( m_buffer.unsentData() ?
			POLLOUT : 0 );
...
namespace
{
	ConnectionSpec parseConnection( ConnectionSpec::Direction direction, char* arg)
	{
		...
		{
			if ( connect_params[3].find( 'a')!=string::npos)
				result.m_flags|=ConnectionSpec::ACK_PACKETS;
			if ( connect_params[3].find( 'p')!=string::npos)
				result.m_flags|=ConnectionSpec::PROMISCUOUS;
		}
		return result;
	}
}
...
RefPtr<StdioTunnel> StdioTunnel::CreateTunnel( int argc, char* argv[])
{
	...
	RefPtr<StdioTunnel> tunnel=local ? static_cast<StdioTunnel*>( new StdioTunnelLocal()) : static_cast<StdioTunnel*>( new StdioTunnelRemote());
...
MagicStringDetector::MagicStringDetector( StdioTunnelLocal& tunnel,
	string magic_string)
...
void StdioTunnelLocal::startHandshaking()
{
	...
	for ( map<int,TunnelConnectionPtr>::iterator it=m_connection_map.begin();
		it!=m_connection_map.end();
		++it)
	{
...
void StdioTunnelLocal::configure( int argc, char* argv[])
{
	...
	for ( int i=1; i<argc; ++i)
	{
		...
		if ( arg[0]=='-' && arg[1]!=0)
		{
			...
			{
			case 'R' :
				m_connection_specs.push_back( parseConnection( ConnectionSpec::LISTEN, 
					long_arg ? arg+2 : argv[i+1]));
				break;
			case 'L' :
				m_connection_specs.push_back( parseConnection( ConnectionSpec::CONNECT, 
					long_arg ? arg+2 : argv[i+1]));
				break;
			case 'e' :
	...
	if ( m_connection_specs.size()==0)
		throw MY_EXCEPTION( "No connections defined");
	if ( m_command.length()==0)
		throw MY_EXCEPTION( "No connection command defined");
}
...
void StdioTunnelLocal::start()
{
	...
	if ( child_pid)
	{
		 ...
		 */
		for ( vector<ConnectionSpec>::iterator i=m_connection_specs.begin();
			i!=m_connection_specs.end();
			++i)
		{
		 ...
		 */
		if ( fcntl( m_read_fd, F_SETFL, O_NONBLOCK)== -1 
			|| fcntl( m_write_fd, F_SETFL, O_NONBLOCK) == -1
			|| fcntl( STDIN_FD, F_SETFL, O_NONBLOCK) == -1
			|| fcntl( STDOUT_FD, F_SETFL, O_NONBLOCK) == -1)
			throw MY_EXCEPTION_ERRNO;
...
void StdioTunnelLocal::polled( Poller&, pollfd& poll_struct)
{
	...
	case PASS_THROUGH :
		if ( poll_struct.fd==m_read_fd)
		{
			...
			if ( m_read_buffer.readData( m_read_fd)==0)
				throw MY_EXCEPTION( "Read side closed");
			if ( m_read_buffer.unsentData())
				getPoller().setEvents( *this, STDOUT_FD, POLLOUT);
		}
		...
		if ( poll_struct.fd==STDIN_FD)
		{
			...
			if ( m_write_buffer.readData( STDIN_FD)==0)
				throw MY_EXCEPTION( "Stdin closed");
			if ( m_write_buffer.unsentData())
				getPoller().setEvents( *this, m_write_fd, POLLOUT);
		}
		if ( poll_struct.fd==STDOUT_FD)
		{
			Trace t( "Write to stdout");
			if ( ! (poll_struct.revents & POLLOUT))
				throw MY_EXCEPTION( "unexpected revents");
	...
	case CONNECTED :
		if ( poll_struct.fd==m_read_fd)
		{
			...
			if ( poll_struct.revents & (POLLERR|POLLNVAL|POLLHUP))
				throw MY_EXCEPTION( "Read pipe closed/invalid");
		...
		{
			if ( poll_struct.revents & (POLLERR|POLLNVAL|POLLHUP))
				throw MY_EXCEPTION( "Write pipe closed/invalid");
...
void StdioTunnelRemote::polled( Poller&, pollfd& poll_struct)
{
	...
	if ( poll_struct.fd==STDIN_FD)
	{
		if ( poll_struct.revents & ( POLLHUP | POLLNVAL | POLLERR))
			throw MY_EXCEPTION( "Stdin closed");
		if ( poll_struct.revents & POLLIN)
			getReader().fill( poll_struct);
		else
		...
		if ( getReader().available())
		{
			...
			case HANDSHAKING :
				if ( command!=CMD_CREATE)
					throw MY_EXCEPTION( "Unexpected handshake command");
				{
...
TunnelConnectionPtr TunnelConnection::CreateConnection( StdioTunnel& tunnel, ConnectionSpec& spec, bool is_local, int id)
{
...
void TunnelConnection::sendData( ConnectionLinkPtr source, char* bytes,
	int length)
{
...
void TunnelConnection::receiveData( StdioTunnel& tunnel, ConnectionLinkPtr link)
{
	...
	if ( length>BUFFER_SIZE)
		throw MY_EXCEPTION( "Bad receive length");
	tunnel.getReader().readBytes( buffer, length);
	if ( ! link->isClosing())
	{
		link->getBuffer().pushOnBuffer( buffer, length);
		tunnel.getPoller().setEvents( link->getPolled(), link->getSocket(),
			POLLOUT);
	}
}
	...
	case LINK_CREATE :
		{
			...
			else
			{
				...
				memcpy( (char*)&dest_addr.sin_addr.s_addr, host_entry->h_addr,
					host_entry->h_length);

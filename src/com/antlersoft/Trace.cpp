...
namespace antlersoft {
  ...
  Trace::Trace(string s) {
    ...
    if (print_it) cerr << "into " << s << cerr_endl() << flush;
  }

  Trace::~Trace() {
    if (print_it) cerr << "out of " << scope_stack.front() << cerr_endl() << flush;
  ...
  void Trace::getTrace() {
    for (deque<string>::iterator i = scope_stack.begin(); i != scope_stack.end(); i++)

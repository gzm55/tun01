...
class RefPtr {
 ...
 public:
  RefPtr() throw() : holder(static_cast<RefObject*>(0)) {}
  ...
  RefPtr(const RefPtr<C>& toCopy) throw() : holder(toCopy.holder) {
    if (holder) holder->m__ReferenceCount++;
  }
  ...
  bool isNull() const throw() { return holder == static_cast<RefObject*>(0); }

  C* operator->() const {
    if (isNull()) throw RefPtrException("Dereferencing null pointer");
  ...
  RefPtr& operator=(const RefPtr<C>& toAssign) throw() {
    if (toAssign.holder) toAssign.holder->m__ReferenceCount++;
    if (holder) {
      if ((--holder->m__ReferenceCount) == 0) {
        delete holder;
      }
